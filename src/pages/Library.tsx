...
import { Plus, Upload, Search, Grid3X3, List } from 'lucide-react';
...
export function Library() {
  ...
  async function handleToggleMod(modId: string, enabled: boolean) {
    try {
      ...
      setMods((prev) =>
        prev.map((mod) => (mod.id === modId ? { ...mod, enabled } : mod))
      );
    } catch (error) {
  ...
  if (hasSearch) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center">
        ...
        <h3 className="text-lg font-medium text-surface-300 mb-1">
          No mods found
        </h3>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-64 text-center">
      ...
      <p className="text-surface-500 mb-4">
        Get started by adding your first mod
      </p>
      <button
        onClick={onInstall}
        className="flex items-center gap-2 px-4 py-2 bg-league-500 hover:bg-league-600 text-white rounded-lg font-medium transition-colors"
